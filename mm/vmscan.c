			  ...
			  struct ctrl_pos *pos)
{
	...
	for (i = tier % MAX_NR_TIERS; i <= umin(tier, MAX_NR_TIERS - 1); i++) {
...
static void inc_max_seq(struct lruvec *lruvec, int swappiness)
{
	int prev, next;
	int type, zone;
	struct lru_gen_folio *lrugen = &lruvec->lrugen;
...
static bool try_to_inc_max_seq(struct lruvec *lruvec, unsigned long max_seq,
			       struct scan_control *sc, int swappiness, bool force_scan)
{
	bool success;
...
static long get_nr_to_scan(struct lruvec *lruvec, struct scan_control *sc, int swappiness)
{
	...
	if (!should_run_aging(lruvec, max_seq, sc, swappiness, &nr_to_scan))
		return nr_to_scan;
		     ...
		     int swappiness, bool force_scan)
{
	...
	return try_to_inc_max_seq(lruvec, max_seq, sc, swappiness, force_scan) ? 0 : -EEXIST;
}
...
unsigned long try_to_free_mem_cgroup_pages_with_swappiness(struct mem_cgroup *memcg,
...
unsigned long try_to_free_mem_cgroup_pages(struct mem_cgroup *memcg,
					   ...
					   unsigned int reclaim_options)
{
	return try_to_free_mem_cgroup_pages_with_swappiness(
		memcg, nr_pages, gfp_mask, reclaim_options, NULL);
}
