 ...
 */
static int memcmpx86_32(void *s1, void *s2, size_t n)
{
	size_t num = n / 4;
	register int res;
 ...
 */
static int memcmpx86_64(void *s1, void *s2, size_t n)
{
	size_t num = n / 8;
	register int res;
...
static void slot_tree_node_assign(struct sradix_tree_node *node,
...
static void slot_tree_node_rm(struct sradix_tree_node *node, unsigned int offset)
{
...
static int slot_iter(void *item,  unsigned long height)
{
...
static void slot_tree_init(void)
{
 ...
 */
static struct page *get_uksm_page(struct ksm_stable_node *stable_node,
				 int unlink_rb, int remove_tree_node)
{
	struct page *page;
 ...
 */
void uksm_remove_vma(struct vm_area_struct *vma, const char *where)
{
	struct vma_slot *slot;
...
static int try_down_read_slot_mmap_sem(struct vma_slot *slot)
{
	...
	if (down_read_trylock(sem)) {
		...
		if (slot_pool_alloc(slot)) {
			uksm_remove_vma(vma, "try_down_read_slot_mmap_sem");
			up_read(sem);
			return -ENOENT;
		}
		return 0;
...
static inline int get_mergeable_page_lock_mmap(struct rmap_item *item)
{
	...
	if (item->slot->vma->anon_vma != folio_anon_vma(folio) ||
 ...
 */
void uksm_add_vma_new(struct vm_area_struct *vma, const char *where)
{
	struct vma_slot *slot;
	unsigned long new_flags = vma->vm_flags;
	unsigned long vm_pages = vma_pages(vma);
			...
			struct page *kpage, pte_t orig_pte)
{
	...
	VM_BUG_ON_PAGE(folio_test_anon(kfolio) && PageAnonExclusive(kpage),
		       kfolio);
				      ...
				      struct page *kpage, u32 hash)
{
	 ...
	 */
	if (write_protect_page(vma, page_folio(page), &orig_pte, NULL) == 0) {
				  ...
				  u32 hash)
{
	...
	if (write_protect_page(vma1, page_folio(page), &wprt_pte1, &orig_pte1) != 0) {
		unlock_page(page);
		goto out;
	}
	...
	if (write_protect_page(vma2, page_folio(tree_page), &wprt_pte2, &orig_pte2) != 0) {
		unlock_page(tree_page);
		goto restore_out;
	}
			       ...
			       struct page *tree_page)
{
	...
	unsigned long address = get_rmap_addr(item);
	DEFINE_FOLIO_VMA_WALK(pvmw, page_folio(kpage), vma, address, 0);
	...
	get_page(tree_page);
	folio_add_anon_rmap_pte(page_folio(tree_page), tree_page, vma, pvmw.address, RMAP_NONE);
	...
	folio_remove_rmap_pte(page_folio(kpage), kpage, vma);
	put_page(kpage);
...
static inline void stable_node_hash_max(struct ksm_stable_node *node,
...
static inline
struct ksm_stable_node *new_stable_node(struct tree_node *tree_node,
		    		        struct page *kpage, u32 hash_max)
{
	...
	folio_set_stable_node(page_folio(kpage), new_stable_node);

	return new_stable_node;
}

static inline
struct ksm_stable_node *first_level_insert(struct tree_node *tree_node,
				           struct rmap_item *rmap_item,
				           struct rmap_item *tree_rmap_item,
				           struct page **kpage, u32 hash,
				           int *success1, int *success2)
{
	...
	struct page *tree_page;
	u32 hash_max = 0;
	struct ksm_stable_node *stable_node, *new_snode;
...
static inline
struct ksm_stable_node *stable_subtree_insert(struct tree_node *tree_node,
					      struct rmap_item *rmap_item,
					      struct rmap_item *tree_rmap_item,
					      struct page **kpage, u32 hash,
					      int *success1, int *success2)
...
static void stable_tree_append(struct rmap_item *rmap_item,
			       struct ksm_stable_node *stable_node, int logdedup)
{
...
int cmp_and_merge_zero_page(struct vm_area_struct *vma, struct page *page)
{
	...
	if (write_protect_page(vma, page_folio(page), &orig_pte, 0) == 0) {
					   ...
					   unsigned long index)
{
	...
	if (pool_index >= slot->pool_size) {
		pr_warn("get_pool_index: vma = %p, pool_index = %lu, pool_size = %lu\n",
			slot->vma, pool_index, slot->pool_size);
		BUG();
	}
	return pool_index;
}
...
void reset_current_scan(struct scan_rung *rung, int finished, int step_recalc)
{
	...
	slot_iter_index = get_random_u32() % rung->step;
			...
			struct vma_slot **slots, unsigned long num)
{
	...
	if (err) {
		/*
		 * If we fail to enter vma slots due to out-of-memory,
		 * we drop the slots to avoid kernel panic. This can happen
		 * during memory pressure, and is better than crashing the kernel.
		 */
		pr_err("UKSM: Failed to enter vma slots with error %d\n", err);
		return;
	}
 ...
 * stable node.
 */
static inline void stable_node_reinsert(struct ksm_stable_node *new_node,
					struct page *page,
...
{
	struct ksm_stable_node *node, *tmp;
...
static void uksm_vma_enter(struct vma_slot **slots, unsigned long num)
{
	...
	/* 
	 * sradix_tree_enter() may fail due to memory allocation,
	 * we should handle this error gracefully instead of panic.
	 */
...
static struct ksm_stable_node *uksm_check_stable_tree(unsigned long start_pfn,
...
static inline int init_random_sampling(void)
{
	...
	for (i = 0; i < HASH_STRENGTH_FULL; i++) {
		...
		swap_index = i + get_random_u32() % rand_range;
		tmp = random_nums[i];
...
static int __init uksm_slab_init(void)
{
	...
	stable_node_cache = UKSM_KMEM_CACHE(ksm_stable_node, 0);
		    ...
		    unsigned long address)
{
	...
	folio_move_anon_rmap(page_folio(page), vma);
...
/* Functions used in ksm.c, but not part of uksm.c. */
