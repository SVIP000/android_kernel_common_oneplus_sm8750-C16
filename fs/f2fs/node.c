...
bool f2fs_available_free_memory(struct f2fs_sb_info *sbi, int type)
{
	...
	/*
	 * give 40%, 40%, 75%, 75%, 40%, 40% memory for each components respectively
	 * Increased thresholds for better background app survival
	 */
	if (type == FREE_NIDS) {
		mem_size = (nm_i->nid_cnt[FREE_NID] *
				sizeof(struct free_nid)) >> PAGE_SHIFT;
		res = mem_size < ((avail_ram * nm_i->ram_thresh / 100) * 40 / 100);
		...
		res = mem_size < ((avail_ram * nm_i->ram_thresh / 100) * 75 / 100);
